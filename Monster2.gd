...
var monster2_tex = preload("res://monster2.png")
var monster2_shadow_tex = preload("res://monster2_shadow.png")

func _ready():
	get_node("/root/Spatial").connect("dimensional_shift", self, "_on_dimensional_shift")

func _on_dimensional_shift(state):
	$texture.set_texture(monster2_tex if state else monster2_shadow_tex)
